...
import com.google.cloud.teleport.v2.options.JdbcToStorageBatchedOptions;
 ...
 */
@Template(
    name = "Jdbc_to_Storage_Batched",
    category = TemplateCategory.BATCH,
    displayName = "JDBC to Cloud Storage (Batched, JSON-in-Avro)",
    description =
        "Pipeline that reads from multiple JDBC tables/queries and writes each to its own "
            + "Cloud Storage path as Avro with a single 'data' field containing JSON.",
    optionsClass = JdbcToStorageBatchedOptions.class,
    flexContainerName = "jdbc-to-storage-batched-v2",
    documentation =
        "https://cloud.google.com/dataflow/docs/guides/templates/provided/jdbc-to-cloud-storage",
...
public class JdbcToStorageBatched {
  ...
  public static void main(String[] args) {
    ...
    JdbcToStorageBatchedOptions options =
  ...
  public static PipelineResult run(JdbcToStorageBatchedOptions options) {
    ...
    for (int i = 0; i < array.size(); i++) {
      ...
      if (!obj.has("table_name") || !obj.has("query") || !obj.has("output_directory")) {
        LOG.error(
            "Invalid tableConfigs[{}]: missing required fields. Skipping: {}", i, obj);
        continue;
      }
      ...
      final String prefix =
          obj.has("output_filename_prefix")
